...
readonly expected_contract_repository='GGRei/v'

if [[ $# -ne 8 ]]; then
	printf '%s\n' 'usage: fork-contract.sh <validator> <contract-root> <bundle-root> <target-id> <canonical-branch> <contract-sha> <bundle-sha> <vc-root>' >&2
	exit 2
fi
...
case "${PUBLISH:-false}:${TCCBIN_PUBLISH:-false}" in
	...
	*) printf '%s\n' 'fork contract probe refuses publication' >&2; exit 1 ;;
esac
...
for exact_member in \
	...
	case "$canonical" in
		*"$exact_member"*) ;;
		*) printf '%s\n' 'manifest fork binding mismatch' >&2; exit 1 ;;
	esac
done
...
printf 'tccbin fork contract target=%s staged=incomplete smoke=v-tcc publish=false: PASS\n' "$target_id"
